...
public class Mutation : ScriptableObject
{
    ...
    public int MaxHeat => Mathf.Clamp(effects.Sum(effect => effect.MaxHeat), AbsoluteMinHeat, AbsoluteMaxHeat);
    public int MaxPh => Mathf.Clamp(effects.Sum(effect => effect.MaxPh),AbsoluteMinPh,AbsoluteMaxPh);
    public int MinPh => Mathf.Clamp(effects.Sum(effect => -effect.MinPh),AbsoluteMinPh,AbsoluteMaxPh);
    public int SupportUV => (int)Mathf.Sign(effects.Sum(effect => effect.SupportUv));
    public int SupportPulsed => (int)Mathf.Sign(effects.Sum(effect => effect.SupportPulsed));
